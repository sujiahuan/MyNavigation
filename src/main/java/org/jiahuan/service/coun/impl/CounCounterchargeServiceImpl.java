...
public class CounCounterchargeServiceImpl extends ServiceImpl<CounCounterchargeMapper, CounCountercharge> implements ICounCounterchargeService {
    ...
    public CounCountercharge getCounCounterchargeByDeviceId(Integer deviceId) {
        ...
        countercharge.setConnetionStatus(iConnectionObj.getConnetionStatusPoll().contains(deviceId));
    ...
    public void openConnection(Integer deviceId) throws Exception {
        ...
        if (iConnectionObj.getConnetionStatusPoll().size() > 5) {
        ...
        BufferedReader bufferedReader = iConnectionObj.getBuffReader(counDevice);
...
//            iCounCounterchargeService.updateById(counCountercharge);
        ...
        Runnable runnable1 = new Runnable() {
            ...
            @Override
            public void run() {
                while (iConnectionObj.getConnetionStatusPoll().contains(counDevice.getId())) {
                    ...
                    switch (counDevice.getAgreement()) {
                        case "05":
                            ...
                            if (countercharge.getResponseParameter() == 9011) {
                                ...
                                customWebSocketConfig.customWebSocketHandler().sendMessageToUser(String.valueOf(counDevice.getId()), new TextMessage("传的RequestResponseCommand参数有问题，请核对：" + countercharge.getResponseParameter() + "\r\n\r\n"));
                            }
                            break;
    ...
    public void addInitByDeviceId(Integer deviceId) {
        CounCountercharge counCountercharge = new CounCountercharge(deviceId, 1, "", 9012, 1);
    ...
    private boolean check05ControlCommand(CounDevice counDevice, String datagram) {
        ...
        switch (getLinkConstant("CN", datagram)) {
            ...
            case "2031":
                ...
                break;
            case "2041":
                regex = "QN=[\\d]{17};ST=" + counDevice.getMonitoringType() + ";CN=2041;PW=[\\w]+;MN=" + counDevice.getMn() + ";Flag=3;CP=&&BeginTime=[\\d]{14},EndTime=[\\d]{14}&&";
                break;
            case "2051":
            ...
            case "3011":
                ...
                break;
            case "3012":
                regex = "QN=[\\d]{17};ST=" + counDevice.getMonitoringType() + ";CN=3012;PW=[\\w]+;MN=" + counDevice.getMn() + ";Flag=3;CP=&&PolID=[\\w]+&&";
                break;
            case "3013":
        ...
        if (!"".equals(regex)) {
            ...
            if (!m.find()) {
                customWebSocketConfig.customWebSocketHandler().sendMessageToUser(String.valueOf(counDevice.getId()), new TextMessage("当前正则为：" + regex + "\r\n"));
                return false;
            }
            return true;
        }
